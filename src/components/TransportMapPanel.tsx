        ...
        {showSettings && <aside className="ui-panel-enter min-w-0 space-y-3">
          ...
          {pinAreaDraft && (
            <section className="rounded-2xl border-2 border-teal-300 bg-white p-4 shadow-sm">
              ...
              <p className="mt-3 text-[10px] leading-relaxed text-slate-500">複数登録できます。上にあるエリアほど、手動で配車を組む際の優先候補として表示します。</p>
          ...
          {zoneDraft && (
            <section className="rounded-2xl border-2 border-violet-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 space-y-3">
                ...
                <label className="flex min-h-11 items-center gap-2 rounded-xl bg-slate-50 px-3 text-sm font-bold"><input type="checkbox" checked={zoneDraft.active} onChange={(event) => setZoneDraft({ ...zoneDraft, active: event.target.checked })} className="h-5 w-5 accent-violet-600" />配車の目安として使用する</label>
