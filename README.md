...
Node.js 24系を使用します（GitHub Actionsと同じ系統）。

```powershell
npm.cmd run check
```

型チェック → 自動テスト → 本番ビルドを順に実行します。個別に確認する場合は `npm.cmd run lint`、`npm.cmd test`、`npm.cmd run build` を使います。

構成の役割分担、整理内容、変更を避けた範囲は[構成レビューと保守方針](docs/構成レビュー_2026-09-07.md)にまとめています。
