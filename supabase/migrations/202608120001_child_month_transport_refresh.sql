-- Rebuild one child's transport requirements without touching other
-- children's manual monthly adjustments.
...
comment on function public.replace_child_monthly_transport_requirements(uuid, date, text, jsonb) is
  'Atomically rebuilds one child month of transport requirements while preserving other children.';
