-- daily_child_plans.child_id and daily_transport_requirements.child_id are text.
-- Replace the original uuid overload so PostgREST exposes the signature that
-- matches the existing schema and the frontend payload.
drop function if exists public.delete_monthly_daily_schedules(uuid, date, uuid);
...
revoke all on function public.delete_monthly_daily_schedules(uuid, date, text) from public;
grant execute on function public.delete_monthly_daily_schedules(uuid, date, text) to authenticated;
grant execute on function public.delete_monthly_daily_schedules(uuid, date, text) to service_role;
