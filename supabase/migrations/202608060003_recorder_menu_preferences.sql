-- Store each actual recorder's navigation choices independently, including
-- recorders who share one authenticated staff account.

alter table public.recorder_profiles
  add column if not exists menu_preferences jsonb not null
  default '{"order":[],"hidden":[]}'::jsonb;

grant select (menu_preferences) on public.recorder_profiles to authenticated;
...
as $$
declare
  v_allowed constant text[] := array['home', 'form', 'records', 'children', 'templates', 'team'];
  v_order_input jsonb;
